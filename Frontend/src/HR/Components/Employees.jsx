...
export default function ManagerEmployeeAttendence() {
  ...
  const fetchEmployees = async () => {
    try {
      setLoading(true);
      // Read HR ID correctly from localStorage
      const hrId = localStorage.getItem("hrID"); // <-- correct key
      if (!hrId) {
      ...
      const res = await axios.get(
        "http://127.0.0.1:8000/attendance/hr-assigned",
        {
          headers: { Authorization: `Bearer ${token}` },
          params: {
            hr_id: parseInt(hrId), // make sure it is a number
  ...
  const fetchDailyAttendance = async () => {
    try {
      const hrId = localStorage.getItem("hrID"); // <-- same key here
      if (!hrId) return;
      ...
      const res = await axios.get(
        ...
        {
          headers: { Authorization: `Bearer ${token}` },
          params: { employee_id: parseInt(hrId), month, year },
        }
      );
