...
export default function EmployeeAttendence() {
  ...
  const fetchProjects = async () => {
    try {
      const managerId = localStorage.getItem("managerId"); // or employeeId/hrId
      if (!managerId) return;

      const res = await axios.get("http://127.0.0.1:8000/attendance/active-projects", {
        headers: { Authorization: `Bearer ${token}` },
        params: { manager_id: parseInt(managerId) } // pass the correct ID
      });
  ...
  const fetchWeeklyAttendance = async () => {
    try {
      ...
      const res = await axios.get("http://127.0.0.1:8000/attendance/weekly", {
        headers: { Authorization: `Bearer ${token}` },
        params: { manager_id: parseInt(managerId) } // pass as query param
      });
